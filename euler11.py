...
data = np.array([list(map(int, line.split())) for line in data.split("\n")])

maximum_value = 0
for i in range(17):
    for j in range(17):
        max_value = max(list(data[i:i+4, j:j+4].prod(axis = 0)) \
        + list(data[i:i+4, j:j+4].prod(axis = 1)) \
        + [np.diag(data[i:i+4, j:j+4]).prod()]\
        + [np.diag(data[i:i+4, j:j+4][::-1]).prod()])
        if max_value > maximum_value:
            maximum_value = max_value
            print(data[i:i+4, j:j+4], i, j)
print(maximum_value)